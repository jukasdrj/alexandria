 ...
 */
export async function handleEnrichEdition(c) {
  try {
    const body = await c.req.json();

    // Validate request
    const validation = validateEnrichmentRequest(body, 'edition');
    if (!validation.valid) {
      return c.json({
        success: false,
 ...
 */
export async function handleEnrichWork(c) {
  try {
    const body = await c.req.json();

    // Validate request
    const validation = validateEnrichmentRequest(body, 'work');
    if (!validation.valid) {
      return c.json({
        success: false,
        error: 'Validation failed',
 ...
 */
export async function handleEnrichAuthor(c) {
  try {
    const body = await c.req.json();

    // Validate request
    const validation = validateEnrichmentRequest(body, 'author');
    if (!validation.valid) {
      return c.json({
        success: false,
        error: 'Validation failed',
 ...
 */
export async function handleQueueEnrichment(c) {
  try {
    const body = await c.req.json();
    ...
    console.log(`📋 Queued ${body.entity_type} enrichment for ${body.entity_key} (queue_id: ${result.queue_id})`);
 ...
 */
export async function handleGetEnrichmentStatus(c) {
  try {
